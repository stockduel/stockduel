...
export default function sellReducer(state, action) {
  ...
  var hasEnoughShares = targetMatch.getIn(['portfolio', 'stocks']).some( (stockObj) => {
    return stockObj.get('stockId') === action.stockId && Number(stockObj.get('shares')) >= Number(action.shares);
  });
  ...
  newStocksArray = UpdatedCash.getIn(['portfolio', 'stocks']).map(function(stock, index) {
      if( stock.get('stockId') === action.stockId ) { // found stock being sold
  ...
  var newState = Map(Object.assign({}, state, {matches: newMatchArray}));
  return newState;
}
